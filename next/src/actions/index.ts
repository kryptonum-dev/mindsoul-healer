...
export async function sendResendMail({ email, message, legal }: ContactProps) {
  ...
  try {
    await resend.emails.send({
      from: `Ali Bahrudinov przez Formularz <formularz@sending.mindsoulhealer.pl>`,
      to: email,
      subject: `Przyjeliśmy twoją wiadomość!`,
      html: `<p>Dzięki za kontakt, spodziewaj się odpowiedzi do 12 godzin.</p>`,
...
export async function createSubscriber({ email, legal }: CreateSubscriberParams) {
  ...
  const params = { email, groups: ['132613531520468195'], fields: { user_token: crypto.randomUUID() } };
